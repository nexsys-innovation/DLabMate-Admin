...
const EntityDetailPage = ({ type }) => {
  ...
  useEffect(() => { const path = type === 'lab' ? `/api/admin/labs/${id}` : `/api/admin/directory/clinics/${id}`; apiRequest(path).then(setData).catch((e) => setError(e.message)); }, [id, type]);
  if (error) return <div className="form-error">{error}</div>; if (!data) return <div className="loading-card">Loading account…</div>;
  ...
  return <><header className="admin-page-head"><div><span className="eyebrow">{type} profile</span><h1>{name}</h1><p>{email}</p></div><span className={`status ${String(entity.registrationStatus).toLowerCase().replaceAll(' ', '-')}`}>{entity.registrationStatus}</span></header>
