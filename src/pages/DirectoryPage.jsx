...
const DirectoryPage = ({ type }) => {
  ...
  const [search, setSearch] = useState(''); const [verificationStatus, setVerificationStatus] = useState(''); const error = '';
  const params = new URLSearchParams({ page: '1', limit: '50' });
  if (search) params.set('q', search);
  if (verificationStatus) params.set('verificationStatus', verificationStatus);
  const path = type === 'lab' ? '/api/admin/labs' : '/api/admin/directory/clinics';
  const query = useCachedQuery([`${path}?${params}`], async () => {
    const data = await apiRequest(`${path}?${params}`);
    return data[plural] || [];
  });
  const items = query.data ?? [];
  
  return <><QueryNotice error={query.error} refresh={query.refresh} hasData={query.data !== undefined} /><header className="admin-page-head"><div><span className="eyebrow">Organization directory</span><h1>{type === 'lab' ? 'Labs' : 'Clinics'}</h1><p>Browse account details, verification status, and case volume.</p></div>{type === 'clinic' ? <Link className="primary-link" to="/clinics/new"><Plus size={17} />Add clinic</Link> : null}</header>
